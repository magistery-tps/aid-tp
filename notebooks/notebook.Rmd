...
km_predictions <- kmeans_predict(ds_step_6, max_min_scale, max_silhouette_k)
...
```{r}
distances_matrix <- dist(x = ds_hc, method = "euclidean") 
```
...
```{r, fig.align='center', fig.height = 6, fig.width = 10}
plot_dendrogram(hc_average_model,  n_clusters)
```

```{r}
hc_predictions <- hc_predict(hc_average_model, ds_hc, n_clusters)
```
