...
best_xgb_test_pred  <- xgboost_predict(xgboost_model, feat(test_set),  xgb_threshold)


table(best_xgb_test_pred, target(test_set))
```


```{r}
test_observation_with_result_type <- observations_cm(
  test_set,
  best_xgb_test_pred,
  target_col = 'Hazardous'
)

test_observation_with_result_type %>% 
  group_by(result_type) %>% 
  tally()
```

```{r}
test_observation_with_result_type %>% filter(result_type == 'FN')
```

```{r}
test_observation_with_result_type %>% filter(result_type == 'FP')
```
