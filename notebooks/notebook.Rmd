...
```{r}
n_clusters <- n_best_features

km_result <- kmeans(t(scale(feat(ds_step_4))), n_clusters)

df_variables <- data.frame(t(feat(ds_step_4)))
df_variables$cluster <- km_result$cluster

variable_clusters <- index_as_column(df_variables) %>% 
 dplyr::select(cluster, index) %>%
 dplyr::rename(Cluster = cluster, Variable = index) %>%
 arrange(Cluster)

datatable(variable_clusters)
```
...
```{r}
ds_step_6 <- filter_outliers_m1(ds_step_5, max_score = 0.51)
